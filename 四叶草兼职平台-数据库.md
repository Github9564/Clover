...
2. 在招聘信息表（cp_position）中添加工作时间描述（job_time_describe）、工作所在区域（job_region ）
3. 在用户表(cp_user_info)中添加昵称(nick_name)、头像（imgs）、系统用户ID（user_base_info_id）
4. 在搜索记录表（cp_search_record ）中添加工作地点（job_place）、结算方式id(cp_settlement_mode_id)、一级分类id（position_first_classify_id），二级分类id（position_second_classify_id）
...
| end_time                    | 工作截止时间   | datetime      |          | N              | Y            |                                    |
| job_time_describe           | 工作时间描述   | int           | 2        | N              | Y            | 1-长期可做 2-指定时间              |
| job_region                  | 工作所在区域   | int           | 2        | N              | Y            | 1.学校附近2.孝南区3.东城区4.老城区 |
| job_place                   | 工作详细地点   | varchar       | 255      | N              | Y            |                                    |
| describe                    | 职位描述       | varchar       | 2000     | N              | Y            |                                    |
| recruit_number              | 招聘人数       | int           | 100      | N              | Y            |                                    |
| wages                       | 工资           | decimal(10,2) | 10       | N              | Y            |                                    |
| wages_units                 | 工资结算单位   | int           | 2        | N              | Y            | 1-时薪2-日薪3-周薪4-月薪           |
...
| position_first_classify_id  | 职位一级分类id | varchar       | 36       | N              | N            |                          |
| position_second_classify_id | 职位二级分类id | varchar       | 36       | N              | N            |                          |
| cp_settlement_mode_id       | 结算方式id     | varchar       | 36       | N              | N            |                          |
| keyword                     | 关键字         | varchar       | 255      | N              | Y            |                          |
| region                      | 工作所在区域   | varchar       | 255      | N              | Y            |                          |
| max_wages                   | 最高工资       | decimal(10,2) |          | N              | Y            |                          |
| min_wages                   | 最低工资       | decimal(10,2) |          | N              | Y            |                          |
| wages_company               | 工资结算单位   | int           | 2        | N              | Y            | 1-时薪2-日薪3-周薪4-月薪 |
| update_time                 | 更新日期       | datetime      |          | N              | Y            |                          |
| create_time                 | 创建日期       | datetime      |          | N              | Y            |                          |
